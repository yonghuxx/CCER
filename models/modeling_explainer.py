...
#这段代码定义了一个函数 repeat_kv，用于将隐藏状态张量 hidden_states 进行维度扩展与重复操作，使得 num_key_value_heads 的头重复若干次，形成 num_attention_heads。
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        #每个key_value头要共享多少个query头
        self.num_key_value_groups = self.num_heads // self.num_key_value_heads
        self.max_position_embeddings = config.max_position_embeddings
        #旋转编码
        self.rope_theta = config.rope_theta
        #因果型注意力，即只关注当前及以前的token
        self.is_causal = True
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            ...
            #causal_mask有的部分是很大的负数，加后会使注意力权重的部分接近零
...
#模块的权重不变，唯一要改的是forward过程，在前向传播中需要正确调用Flash Attention的API，并处理输入中可能存在的填充标记
#FlashAttention能提高显存利用率，计算效率
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return attn_output, attn_weights, past_key_value
    #通过Flash Attention API进行注意力计算，如果输入序列中包含填充标记，函数会将填充部分去掉
    #再进行注意力得分计算，最后将计算出的注意力得分重新加上填充部分，返回最终输出
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if (inputs_embeds is not None and past_key_values is None) or (input_ids.shape[-1]==0):
...
#用序列中的最后一个token用来分类，因为模型会将上下文信息全部聚合到最后一个token
